...
from .py3compat import (tobytes, bord, _copy_bytes, iter_range,
                                  tostr, bchr, bstr)

from . import SHA1, SHA256, HMAC, CMAC, BLAKE2s
from .strxor import strxor
from .Random import get_random_bytes
from .number import size as bit_size, long_to_bytes, bytes_to_long

from ._raw_api import (load_pycryptodome_raw_lib,
                                  create_string_buffer,
                                  get_raw_buffer, c_size_t)

_raw_salsa20_lib = load_pycryptodome_raw_lib("._Salsa20",
                    """
                    int Salsa20_8_core(const uint8_t *x, const uint8_t *y,
                                       uint8_t *out);
                    """)

_raw_scrypt_lib = load_pycryptodome_raw_lib("._scrypt",
                    """
...
def PBKDF1(password, salt, dkLen, count=1000, hashAlgo=None):
    ...
    Args:
     ...
     dkLen (integer):
        ...
        instance for :mod:`.AES`.
     count (integer):
        The number of iterations to carry out. The recommendation is 1000 or
        more.
     hashAlgo (module):
        The hash algorithm to use, as a module or an object from the :mod:`.` package.
        The digest length must be no shorter than ``dkLen``.
        The default algorithm is :mod:`.SHA1`.
...
def HKDF(master, key_len, salt, hashmod, num_keys=1, context=None):
    ...
    Args:
     ...
     hashmod (module):
        A cryptographic hash algorithm from :mod:`.`.
        :mod:`.SHA512` is a good choice.
     num_keys (integer):
