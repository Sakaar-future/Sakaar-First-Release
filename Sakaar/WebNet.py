...
def AddNewWallet():
    ...
    while (True):
        ...
        if tcp_http == "TCP":
            ...
            break
    Wallet_Out(Symbol, Registr_API, Registr_API_1, Registr_API_R, Registr_API_R_1, Registr_API_R_2, GetBalance_API,
               GetBalance_API_1, GetBalance_API_R, GetBalance_API_R_1, Send_API, Send_API_1, Send_API_2, Send_API_3,
               Send_API_4, Send_API_R, tcp_http, http_reg, http_bal, http_sen, tcp_address, tcp_port)
    DataToConnect = {}
    DataToConnect['Symbol'] = Symbol
    DataToConnect['Registr_API'] = Registr_API
    DataToConnect['Registr_API_1'] = Registr_API_1
    DataToConnect['Registr_API_R'] = Registr_API_R
    DataToConnect['Registr_API_R_1'] = Registr_API_R_1
    DataToConnect['Registr_API_R_2'] = Registr_API_R_2
    DataToConnect['GetBalance_API'] = GetBalance_API
    DataToConnect['GetBalance_API_1'] = GetBalance_API_1
    DataToConnect['GetBalance_API_R'] = GetBalance_API_R
    DataToConnect['GetBalance_API_R_1'] = GetBalance_API_R_1
    DataToConnect['Send_API'] = Send_API
    DataToConnect['Send_API_1'] = Send_API_1
    DataToConnect['Send_API_2'] = Send_API_2
    DataToConnect['Send_API_3'] = Send_API_3
    DataToConnect['Send_API_4'] = Send_API_4
    DataToConnect['Send_API_R'] = Send_API_R
    DataToConnect['tcp_http'] = tcp_http
    DataToConnect['http_reg'] = http_reg
    DataToConnect['http_bal'] = http_bal
    DataToConnect['http_sen'] = http_sen
    DataToConnect['tcp_address'] = tcp_address
    DataToConnect['tcp_port'] = tcp_port
    return DataToConnect


class Wallet_Out:
    def __init__(self, Symbol, Registr_API, Registr_API_1, Registr_API_R, Registr_API_R_1, Registr_API_R_2,
                 GetBalance_API, GetBalance_API_1, GetBalance_API_R, GetBalance_API_R_1, Send_API, Send_API_1,
                 Send_API_2, Send_API_3, Send_API_4, Send_API_R, tcp_http, http_reg, http_bal, http_sen, tcp_address,
                 tcp_port):
        conn = sql3connect(Path + '/Out_Data_Base.db')
        c = conn.cursor()
        c.execute('''CREATE TABLE IF NOT EXISTS API_Wallet(
        ...
        if c.fetchone() is None:
            c.execute(
                f'''INSERT OR IGNORE INTO API_Wallet VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                (
                    Symbol,
                    Registr_API,
                    Registr_API_1,
