...
def DeleteIP_TCP():
    if conf.conf['MyIP_TCP'] != None and conf.conf['MyIP_TCP'] in conf.conf['Connected_TCP'] and conf.conf['MyIP_TCP'][2] == conf.conf['login'] :
        DelIP_TCP(conf.conf['MyIP_TCP'],PrivCode(decode(json.dumps(['DelIP',conf.conf['MyIP_TCP'][0],conf.conf['MyIP_TCP'][1],conf.conf['login']]),256),conf.conf['PrivKey']))
def AddMyIP_TCP():
    if not(conf.conf['In']):
        return
    if 'MyIP_TCP' not in conf.conf or conf.conf['MyIP_TCP'] == None or conf.conf['MyIP_TCP'][2] != conf.conf['login']:
        ...
        NewIP_TCP(conf.conf['MyIP_TCP'],PrivCode(decode(json.dumps(['NewIP',ip,port,conf.conf['login']]),256),conf.conf['PrivKey']))
    elif conf.conf['MyIP_TCP'] not in conf.conf['Connected_TCP'] and conf.conf['MyIP_TCP'][2] == conf.conf['login']:
        NewIP_TCP(conf.conf['MyIP_TCP'], PrivCode(decode(json.dumps(['NewIP',conf.conf['MyIP_TCP'][0],conf.conf['MyIP_TCP'][1],conf.conf['login']]),256),conf.conf['PrivKey']))
def DeleteIP():
    if conf.conf['MyIP'] != None and conf.conf['MyIP'] in conf.conf['Connected'] and conf.conf['MyIP'][1] == conf.conf['login'] :
        DelIP(conf.conf['MyIP'],PrivCode(decode(json.dumps(['DelIP',conf.conf['MyIP'][0],conf.conf['login']]),256),conf.conf['PrivKey']))
def AddMyIP():
    if 'MyIP' not in conf.conf or conf.conf['MyIP'] == None or conf.conf['MyIP'][1] != conf.conf['login']:
        ip = input("IP () : ")
        conf.conf['MyIP'] = [ip,conf.conf['login']]
        NewIP(conf.conf['MyIP'],PrivCode(decode(json.dumps(['NewIP',ip,conf.conf['login']]),256),conf.conf['PrivKey']))
    elif conf.conf['MyIP'] not in conf.conf['Connected'] and conf.conf['MyIP'][1] == conf.conf['login']:
        NewIP(conf.conf['MyIP'],PrivCode(decode(json.dumps(['NewIP',conf.conf['MyIP'][0],conf.conf['login']]),256),conf.conf['PrivKey']))
    conf.conf.close()
    get_conf()
...
def Server_Proc_TCP(sock):
    ...
    try:
        ...
        elif dat['Protocol'] == 'getBalance_list':
            res = getBalance_list_R(dat)#
        ...
        elif dat['Protocol'] == 'generate_Private':
            res = generate_Private_R(dat)
        elif dat['Protocol'] == 'PubToAdr':
            res = PubToAdr_R(dat)
        elif dat['Protocol'] == 'AdrToPub':
            res = AdrToPub_R(dat)
        elif dat['Protocol'] == 'PrivToPub':
            res = PrivToPub_R(dat)
        elif dat['Protocol'] == 'PubCode':
            res = PubCode_R(dat)
        elif dat['Protocol'] == 'PrivCode':
            res = PrivCode_R(dat)
        else:
            tor = 0
        Send_TCP(res,sock)
        ...
        if dat['Protocol'] == 'SendTranzh':
            ...
            NewIP_R(dat)
        elif dat['Protocol'] == 'DelIP_TCP':
            DelIP_TCP_R(dat)
        elif dat['Protocol'] == 'NewIP_TCP':
            NewIP_TCP_R(dat)
            ...
            for file in os.listdir(dirs):
                if os.path.isfile(dirs+'//'+file):
                    with open(dirs+'//'+file, 'rb') as input:
                        if file  != '.DS_Store' and file != 'exmp1.db' and file != 'VersionKey.py':
                            dat['files'].append([(dirs+'//'+file)[arc:],str(b64encode(input.read()), 'utf-8',errors='ignore').strip()])
                elif file != '__pycache__':
                    indir(dirs+'//'+file,arc)
        indir(Path,len(Path)-6)
        dat['files'] = sorted(dat['files'])
        import Sakaar.VersionKey
        # print(PubCode(int(VersionKey.Key),AdrToPub(conf.conf['Key'])))
        if decode(sha256_16(sha256_16(dat)+conf.conf['Version']),16) != PubCode(int(VersionKey.Key),AdrToPub(conf.conf['Key'])):
            # pass
            raise "Incorrect version of soft"
def UpDate(Ver):
...
def SaveTranzhs(Tranzh):
    ...
    for T in Tranzh['TS']:
        ...
        c.execute(f'''INSERT INTO History VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (conf.conf['Block'], PubToAdr(T['PubKey1']), PubToAdr(T['PubKey2']), T['Sum'], T['Comis'], T['Wal'], T['time'], mas[T['Wal']][T['PubKey1']]['AddressTo'], mas[T['Wal']][T['PubKey2']]['AddressTo'], json.dumps(T['Comis']),json.dumps(int(T['MesIn'])),json.dumps(int(T['MesOut']))))
        conn.commit()
    for wal in mas:
        for name in mas[wal]:
            name = mas[wal][name]
...
def EndTranzh(Tranzh, res):
    ...
    if conf.conf['isServer']:
        ...
        if tor :
            ...
            c = conn.cursor()
            for T in Tranzh['TS']:
                c.execute(f'''DELETE FROM ATran WHERE PubKey1 = '{PubToAdr(T['PubKey1'])}' AND PubKey2 = '{PubToAdr(T['PubKey2'])}' AND Wallet = '{T['Wal']}' AND Time = {T['time']} ''')
            conn.commit()
            conn.close()
...
def SendTranzh(x):
    ...
    if conf.conf['isServer']:
        ...
        c = conn.cursor()
        for T in x['TS']:
            c.execute(f'''INSERT INTO ATran VALUES(?, ?, ?, ?, ?, ?, ?)''', (PubToAdr(T['PubKey1']), PubToAdr(T['PubKey2']), T['Sum'], T['Comis'], T['Wal'], T['time'], get_User(PubToAdr(T['PubKey1']),T['Wal'])['AddressTo']))
            conn.commit()
        conn.close()
        tor = 1
...
def getHTran(Wal, Address,t = None,ofPub = 1):#exteranl Address
    ...
    if conf.conf['isServer']:
        ...
        if t == None:
            if(ofPub == 1):
                dat = c.execute(f''' SELECT * FROM History WHERE (PubKey2 = '{Address}' OR PubKey1 = '{Address}') AND Wallet = '{Wal}' ORDER BY Time''').fetchall()
            else:
                dat = c.execute(f''' SELECT * FROM History WHERE (Address1 = '{Address}' OR Address2 = '{Address}') AND Wallet = '{Wal}' ORDER BY Time''').fetchall()
        else:
            if(ofPub == 1):
                dat = c.execute(f''' SELECT * FROM History WHERE (PubKey2 = '{Address}' OR PubKey1 = '{Address}') AND Wallet = '{Wal}' AND Time > {t} AND Wallet = '{Wal}' ORDER BY Time LIMIT 0,50''').fetchall()
            else:
                dat = c.execute(f''' SELECT * FROM History WHERE (Address1 = '{Address}' OR Address2 = '{Address}') AND Wallet = '{Wal}' AND Time > {t} AND Wallet = '{Wal}' ORDER BY Time LIMIT 0,50''').fetchall()
        conn.close()
        return dat
...
if 'Connected' not in conf.conf:
    conf.conf['Connected'] = [['2de0e412453b.ngrok.io','GrandBull']]
...
if 'Key' not in conf.conf:
    conf.conf['Key'] = 'AAAAgQCVyHesUOzUjebFgOfNy9kc0EfqmU8FJotUt4eO9JtdZ8pwBst0jx/p4Pcve/10zWCjYEQ5Iy/I37s30KjmKz0DiehxFJ79Mq8L8oDfY3i6uPOIk9kONeLYoovLjn/jUVYwfwYNJeNp6Q+bnqQkaTv+vAKECyoK3k0ThsURzkrUWw=='
